...
export default function SetupChecklist() {
  ...
  return (
    <div className="border border-line rounded-xl bg-white p-5 flex flex-col gap-3" data-testid="setup-checklist">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
        {d.items.map((i: any) => (
          <Link key={i.key} href={i.href} className={`flex items-center gap-2 text-[13px] rounded-lg px-2 py-1.5 hover:bg-paper ${i.done ? "text-ink-soft line-through" : "font-semibold"}`}>
            <span className={`w-4 h-4 rounded-full border flex items-center justify-center text-[10px] ${i.done ? "bg-signal border-signal text-white" : "border-line"}`}>{i.done ? "✓" : ""}</span>
            {i.label}
          </Link>
        ))}
      </div>
