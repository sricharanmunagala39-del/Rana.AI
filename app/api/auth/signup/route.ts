...
export async function POST(req: Request) {
  ...
  await sendEmail({ to: hqInbox(), kind: "signup", clientId: client.id, subject: `New sign-up waiting: ${company}`, html: emailHtml({ title: `${company} wants a free trial`, lines: [`${esc(name || "—")} · ${esc(email)} · ${esc(phone)}`, "Approve it in RANA HQ to start their 14-day trial."], button: { label: "Open RANA HQ", url: `${APP_URL()}/hq` } }) });
